...
public class Animal_Horse : Animal
{
    ...
    [Header("冲刺冲量检测")]
    [Tooltip("冲刺检测中心偏移（相对于自身位置）")]
    public float dashHitRangeOffset = 0.5f;
    [Tooltip("冲刺检测范围半径")]
    public float dashHitRangeRadius = 1.0f;
    [Tooltip("冲刺冲量大小")]
    public float dashImpulseForce = 1f;
    [Tooltip("冲刺时移动速度倍率")]
    [Range(1f, 5f)]
    ...
    private void OnDrawGizmosSelected()
    {
        ...
        // 绘制冲刺检测范围
        float facing = (animalSprite != null && animalSprite.flipX) ? -1f : 1f;
        Vector3 center = transform.position + Vector3.right * (facing * dashHitRangeOffset);
        
        Gizmos.color = new Color(0f, 0.8f, 1f, 0.5f);
        Gizmos.DrawWireSphere(center, dashHitRangeRadius);
        Gizmos.color = new Color(0f, 0.8f, 1f, 0.15f);
        Gizmos.DrawSphere(center, dashHitRangeRadius);
    }
#endif
}
